...
//    protected $userClass = '\SimpleUser\User';
    protected $userClass = '\SimpleUser\JWT\User';
...
//        new User(); \SimpleUser\JWT\User
    ...
    public function findOneBy(array $criteria)
    {
        ...
        $response = reset($users);

        printToTerminal(json_encode('users'));
        printToTerminal(json_encode($users));
        printToTerminal(json_encode('reset'));
        printToTerminal(json_encode($response));
        return $response;
...
//        printToTerminal($sql);
//        printToTerminal($params);
//        $data = $this->conn->fetchAll($sql, $params,array($this->getUserColumns('email') => 's'));
        $data = $this->conn->executeQuery($sql, array($params['email']))->fetchAll();
...
//                $sql .= ($first_crit ? 'WHERE' : 'AND') . ' ' . $key . ' = :' . $key . ' ';
    ...
    public function getUserColumns($column = ""){
        ...
        else return substr($this->userColumns[$column], 1, -1);
    }
